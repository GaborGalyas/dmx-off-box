...
DEVICEID _devID = { 0x4b, 0x00, 0x20, 0x12, 0x00, 0x00 };

// The Device ID for addressing all devices of a manufacturer.
DEVICEID _devIDGroup = { 0x4b, 0x00, 0xFF, 0xFF, 0xFF, 0xFF };
...
void DMXSerialClass2::_processRDMMessage(byte CmdClass, uint16_t Parameter, bool8 handled, bool8 doRespond)
{
  ...
  if (! handled ) {
    ...
    if (Parameter == SWAPINT(E120_IDENTIFY_DEVICE)) { // 0x1000
      ...
      if (CmdClass == E120_GET_COMMAND) {
        if (_rdm.packet.DataLength > 0) {
          ...
          devInfo->productCategory = SWAPINT(E120_PRODUCT_CATEGORY_POWER_CONTROL);
	...
	else if (Parameter == SWAPINT(E120_DMX_START_ADDRESS)) {
				// 0x00F0
      if (CmdClass == E120_SET_COMMAND) {
        if (getFootprint() == 0) {
...
#ifdef USE_DIPSWITCH
		else if (dipaddress() != 0) {
          // No sub-devices supported
          nackReason = E120_NR_WRITE_PROTECT;
        } 
#endif
		
		else {
          uint16_t newStartAddress = READINT(_rdm.packet.Data);
          if ((newStartAddress <= 0) || ((newStartAddress + getFootprint() -1) > DMXSERIAL_MAX)) {
        ...
        if (_rdm.packet.DataLength > 0) {
          ...
          WRITEINT(_rdm.packet.Data, dipaddress());
          _rdm.packet.DataLength = 2;
          handled = true;;
        } 
#endif		
      ...
      if (CmdClass == E120_GET_COMMAND) {
        if (_rdm.packet.DataLength > 0) {
          ...
          WRITEINT(_rdm.packet.Data+ 6, E120_RESET_DEVICE);
          WRITEINT(_rdm.packet.Data+ 8, E120_LAMP_HOURS);
          uint8_t offset = 10;
...
int DMXSerialClass2::dipaddress(){
dipaddr = 0;
//3, 4, 5, 6, 7, 8, 9, 10, 11
  if (digitalRead(DIP1) != 1){dipaddr = dipaddr+1;}
  if (digitalRead(DIP2) != 1){dipaddr = dipaddr+2;}
  if (digitalRead(DIP3) != 1){dipaddr = dipaddr+4;}
  if (digitalRead(DIP4) != 1){dipaddr = dipaddr+8;}
  if (digitalRead(DIP5) != 1){dipaddr = dipaddr+16;}
  if (digitalRead(DIP6) != 1){dipaddr = dipaddr+32;}
  if (digitalRead(DIP7) != 1){dipaddr = dipaddr+64;}
  if (digitalRead(DIP8) != 1){dipaddr = dipaddr+128;}
  if (digitalRead(DIP9) != 1){dipaddr = dipaddr+256;}
  return(dipaddr);
}
#endif
